...
impl Graph {
    pub fn new(expr_bank: &ExprBank) -> Self {
        let nodes = vec![Self::add_node(&ExprRef(expr_bank.0.len() - 1), &expr_bank)];
        Self { nodes }
    }
        ...
        match expr {
            ...
            Expr::Combinator(combinator) => match combinator {
                Combinator::Chain(left_ref, right_ref) => {
                    let left = Self::add_node(left_ref, expr_bank);
                    let mut right = Self::add_node(right_ref, expr_bank);
